...
### Shares
- `GET    /api/shares` → `{smb:[{name, path, comment?, readOnly, guestOk, validUsers:string[], readList:string[], recycle:boolean, managed:boolean}], nfs:[{path, clients:[{host, options}]}]}`. Lists ALL smb.conf share sections (minus Samba's special `global`/`homes`/`printers`/`print$`); `managed:false` marks hand-authored shares outside proxsyno's markers (surfaced read-only — the UI hides edit/delete). `readList` are users/`@groups` forced read-only; `recycle` reflects a `vfs objects = recycle` block. `POST`/`PUT` refuse to shadow an unmanaged section of the same name (409).
- `POST   /api/shares/smb` `{name, path, comment?, readOnly?, guestOk?, validUsers?:string[], readList?:string[], recycle?:boolean}` → `201 {share}`. Writes a managed block in `/etc/samba/smb.conf` and reloads smbd. `validUsers`/`readList` entries accept `name` or `@group`. When `recycle` is set, deletions move to `<path>/.recycle` (keeptree + versions).
