...
// Above this many items the grid is row-windowed inside a bounded scroll box.
// At or below it, render the plain responsive grid exactly as before.
const VIRTUALIZE_THRESHOLD = 80;
const GAP = 8; // matches gap-2

// Columns per row, matching the Tailwind grid-cols breakpoints used below
...
export function PhotoGrid({
  ...
  // Measure the scroll container's width (before paint) so we can derive the
  // number of columns and the square cell/row height for windowing. When not
  // windowing, scrollRef is null and this stays inert.
  ...
  const rowH = cellW > 0 ? cellW + GAP : 200;
  ...
  // Row height / column count change on resize — re-measure the virtualizer.
  useEffect(() => {
    virtualizer.measure();
  }, [rowH, cols, virtualizer]);

  // Single cell renderer shared by the plain and windowed grids so the markup
  // stays identical. `index` is the absolute item index (for onOpenItem).
  const renderItem = (item: MediaItem, index: number) => {
    ...
    return (
      ...
      >
        <button onClick={() => onOpenItem(index)} className="block h-full w-full" title={item.name}>
          {showThumb ? (
            <img
            ...
            <div style={{ height: virtualizer.getTotalSize(), position: 'relative' }}>
              {virtualizer.getVirtualItems().map((vr) => {
                ...
                return (
                  ...
                  >
                    {items.slice(start, start + cols).map((item, j) => renderItem(item, start + j))}
                  </div>
                );
              })}
            </div>
